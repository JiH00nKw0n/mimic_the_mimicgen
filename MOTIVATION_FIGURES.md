...
- 모방학습은 물체 자세·배치·주변 클러터에 따라 필요한 동작이 달라지므로, 소수의 데모만으로는 deploy(배포) 시 마주하는 다양한 상황과 그에 따른 다양한 동작을 충분히 학습하기 어렵고, 따라서 조건별 변화를 담은 크고 다양한 데모 데이터셋이 필요하다.
- 실제 로봇으로 이런 데이터를 직접 모으는 것은 비용과 시간이 많이 들어, 소수의 human 데모를 transform·stitch·retain 파이프라인으로 재활용해 다양한 조건의 합성 데모를 자동 생성하는 연구가 활발하다.
- 이때 학습에는 task에 성공해 retain된 데모만 쓰이므로, retained 데이터셋이 조건별 다양성을 충분히 보존해야 한다.
