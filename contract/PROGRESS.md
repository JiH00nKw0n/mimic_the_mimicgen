...
우리 재실행은 중력을 켠 채(솔버 8회, 한계 미설정) 돌렸는데, 강성 0의 순수 토크 팔은
보상되지 않는 중력에 처지므로 이것이 4.8 cm 잔차의 유력한 원인이다. 이 설정 전부를
`warmstart_replay.py`에 반영했고(`--robot_usd` 기본값 = 래퍼 자산), 서버가 다시 켜지면
`run_warmstart_aidas.sh` 한 번으로 재검증한다. 계약 코드 3종(actions_cfg,
task_space_actions, contract yaml)은 패키지본과 우리 vendored본이 바이트 단위로 동일함을
확인했다.
...
2. 서버 재기동 시 warm-start 재실행(RL 런타임 설정 반영본) → 잔차·태스크 성공 재검증으로
   계약 정합 종결. RL팀 체크리스트 답변은 `mimicgen_inputs_supplied.yaml`로 작성해 둠.
