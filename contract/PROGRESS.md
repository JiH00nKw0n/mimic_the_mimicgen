...
- **MimicGen**: 소수의 시연을 수백에서 수천 개로 증폭하는 방법. 시연을 "물체에 접근 → 잡기 →
...
성분(약 3.4 cm)"으로 분해된다. 속도 비례 성분은 계약 제어기의 의도된 과감쇠 특성
(τ≈0.42 s; 사전 기록된 델타를 재생하면 지연 = 속도 × τ가 필연이고, 실제 RL에서는 폐루프
정책이 이를 보정한다)으로 설명된다. 대조 실험으로 RL 시연(이 제어기 위에서 녹화된
데이터, 가장 빠름)을 같은 방식으로 재실행하면 잔차가 10.9 cm로 가장 크다 — 세 측정점
(5.4/7.5/10.9 cm)이 같은 모델에 맞으므로, "기록 궤적의 개루프 복사"는 이 제어기에서
소스와 무관하게 달성 불가능한 기준이다. 상수 성분 약 3.4 cm는 동결 제어기+자산 스택 내부에
...
| 소스 | 조건 | 수율 | 성공의 소스 분포 |
|---|---|---|---|
| 사람 원본 (13개) | annotation 원본 | 25/385 (6.5%) | 2개 시연 (22+3) |
| 사람 내용 + 합성 annotation (identity) | 합성 코드 검증용 | 10/137 (7.3%) | 1개 시연에 9 |
| RL-teacher 원본 스타일 (50개) | TCP 수정만 | 0/3117 (0%) | — |
| RL-teacher, 재작성 + 2배 감속 | 아래 6-7 | 25/486 (5.1%) | 1개 시연 (27번) |
| RL-teacher, 재작성 + 3배 감속 | 6-7 + 캡 60 s | 10/118 (8.5%) | 2개 시연 (27번 8 + 11번 2) |

(초기 소표본 측정은 사람 10/122=8.2%, RL 재작성 10/285=3.5%였고, 표본을 키운 위 수치가
최종이다.) 세 가지 결론이 선다. 첫째, identity가 사람 원본과 같은 수율을 내므로
annotation 합성 코드는 결함이 없고, RL의 0%는 전부 RL 시연 스타일과 변환 결함에서 나온
것이다(원인 사슬은 6-7). 둘째, 재작성에 3배 감속까지 얹으면 RL 소스도 사람 수율에
도달한다(8.5% vs 6.5%, 신뢰구간 겹침). 셋째, 성공의 소스 편중은 RL만의 약점이 아니라
사람 소스에서도 똑같이 나타나는 이 파이프라인의 공통 특성이다(사람도 25개 중 22개가
시연 1개에서). robosuite 실험에서 확인했던 소스별 생성 수율 편차가 Isaac 파이프라인에서도
재현되는 것이며, 감속을 키우면 활성 소스 수가 늘어나는 방향성도 관찰됐다(1개→2개).
...
7. **RL 0%의 원인 사슬 — 소스 재작성 14단계.** 실패 에피소드를 지표(환경이 직접 계산한
   ...
   - 그리퍼 펌핑(60스텝에 부호 10회 반전; 폐루프에선 운 좋은 사이클에 잡음) → 신호 기반
     깨끗한 픽-플레이스 구형파로 교체.
   - RL 시작 자세가 관절 전부 0에 가까운 수직 특이점 → lab 홈 구성으로 리셋 + 고공 구간 절단.
...
1. 사람 vs RL 비교 규모 확장(성공 25개 목표, 3-cube 단일 태스크로 확정 — peg은 스코프
   제외 결정) + 소스 27번 편중의 원인 분석 — 다음 주 미팅 보고.
2. RL팀에 재실행 잔차 보고(`REPORT_TO_RL_20260803.md`) 전달 → 상수 성분(약 3.4 cm)의
