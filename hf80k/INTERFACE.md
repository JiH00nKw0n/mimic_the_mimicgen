...
컨테이너는 청크가 끝날 때마다 그 청크의 `lerobot/`을 아래 자리에 올린다. 마지막
합치기는 합친 결과를 최상위에 올린다.

```
저장소 최상위/
  meta/  data/  videos/     합친 최종본. LeRobotDataset(repo_id)가 여는 것이 이것이다
  chunks/
    <SHARD_ID>/             컨테이너 한 대의 몫. 예: gpu0, gpu1, seed42000
      chunk_00000/          그 대가 만든 청크 하나
      chunk_00001/
```

`SHARD_ID`를 비우면 `SEED_BASE`에서 이름을 만든다(`seed42000`). 컨테이너를 하나만
띄우고 `SEED_BASE`도 비우면 몫 단계가 사라져 `chunks/chunk_00000`이 된다.

청크 번호는 컨테이너마다 0부터 다시 시작하므로, 몫 이름이 없으면 여러 대가 같은 자리에
올려 서로 덮어쓴다. 오류도 나지 않고 각 대의 `MANIFEST.json`도 전부 `done`이라, 다
끝나고 데이터셋을 열어 봐야 편수가 모자란 것을 알게 된다.
