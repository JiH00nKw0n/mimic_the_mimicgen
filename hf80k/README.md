...
아래는 L40S 1장짜리 박스에서 이 이미지로 50편 청크를 다섯 단계 전부 끝까지 돌려 얻은
값이다. `NUM_ENVS=16`, `GEN_PROCS=1`, `RENDER_PROCS=2`, 물리 랜덤화를 켠 상태다.

| 단계 | 걸린 시간 | 편당 | 비고 |
|---|---:|---:|---|
| 생성 | 578.5초 | 11.6초 | 시도 402회에 성공 50편. 시도당 1.44초, 수율 12.4% |
| 계약 형식 변환 | 69.7초 | 1.4초 | 형식 위반 0건 |
| 렌더 | 281.7초 | 5.6초 | 프로세스 2개가 각각 261.3초와 256.7초. 벽시계는 둘 중 긴 쪽 |
| LeRobot 기록 | 216.1초 | 4.3초 | 영상 3종 H.264 인코딩이 대부분이다 |
| 허깅페이스 업로드 | 3.3초 | 0.07초 | 50편에 45.9 MB |
| 합계 | 1149.3초 | 23.0초 | |

여기서 확인한 것이 세 가지다.

**동시 환경 수를 4에서 16으로 올리면 생성이 2.06배 빨라진다.** 시도당 2.97초에서
1.44초가 됐다. 이 값을 재기 전까지는 MimicGen이 시도를 하나씩 처리하느라 환경을 늘려도
이득이 없을 수 있다고 봤는데, 실제로는 이득이 있다. 기본값 `NUM_ENVS=16`을 그대로 쓰면
된다.

**물리 랜덤화를 켜도 수율이 떨어지지 않는다.** 끄고 잰 값이 15.2%(164시도 중 25성공),
켜고 잰 값이 14.2%(351시도)와 12.4%(402시도)다. 세 값의 차이는 통계적으로 유의하지 않다.
실측 물리를 넣는 대가로 데이터를 잃지 않는다.

**완성된 데이터는 편당 0.92 MB다.** 50편이 45.9 MB로 올라갔다. 8만 편이면 약 73 GB다.

### 추정한 값

위 편당 23.0초를 그대로 늘린 값이다.
...
- 측정에 쓴 청크가 50편이라 Isaac Sim 시작 비용이 편당 10배 크게 잡혀 있다. 한 청크에서
  Isaac을 다섯 번(생성 1, 변환 1, 렌더 2, 기록 1) 띄우는데, 프로덕션 설정인 500편
  청크에서는 같은 시작 비용이 열 배 많은 편수에 나뉜다. 그만큼 실제 값은 위 표보다
  짧다. 얼마나 짧은지는 첫 500편 청크의 `durations_s`를 보면 바로 나온다.
