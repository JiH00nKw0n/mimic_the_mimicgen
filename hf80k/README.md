...
아래는 L40S 1장짜리 박스에서 이 이미지로 50편 청크를 실제로 끝까지 돌려 얻은 값이다.
`NUM_ENVS=4`, `GEN_PROCS=1`, `RENDER_PROCS=2`, 물리 랜덤화를 켠 상태다.

| 단계 | 걸린 시간 | 편당 | 비고 |
|---|---:|---:|---|
| 생성 | 1042.8초 | 20.9초 | 시도 351회에 성공 50편. 시도당 2.97초, 수율 14.2% |
| 계약 형식 변환 | 71.3초 | 1.4초 | 형식 위반 0건 |
| 렌더 | 478.9초 | 9.6초 | 프로세스 2개가 각각 350.7초와 478.9초. 벽시계는 둘 중 긴 쪽 |
| 합계 | 1592.9초 | 31.9초 | 기록·업로드 단계는 아직 이 측정에 없다 |

물리 랜덤화를 켜도 수율이 떨어지지 않는다. 켜기 전이 15.2%(164시도 중 25성공), 켠 뒤가
14.2%(351시도 중 50성공)로 차이가 통계적으로 유의하지 않다. 실측 물리를 넣는 대가로
데이터를 잃는 일은 없다는 뜻이다.

### 추정한 값

위 편당 31.9초를 그대로 늘린 값이다. 동시 환경 수 4개 기준이다.
...
- GPU 여러 장 값은 1장 값을 장수로 나눈 것이다. 컨테이너가 서로 독립이라 계산은 맞지만,
  업로드 대역폭과 디스크 입출력 경합은 넣지 않았다.
- 측정에 쓴 청크가 50편이라 Isaac Sim 시작 비용이 편당 10배 크게 잡혀 있다. 한 청크에서
  Isaac을 네 번(생성 1, 변환 1, 렌더 2) 띄우는데, 프로덕션 설정인 500편 청크에서는 같은
  시작 비용이 열 배 많은 편수에 나뉜다. 그만큼 실제 값은 위 표보다 짧다. 얼마나 짧은지는
  아직 재지 않았다.
- 반대로 기록과 업로드 단계 시간이 위 표에 빠져 있어, 그만큼은 길어진다.
- 두 방향이 얼마나 상쇄되는지는 첫 500편 청크의 `durations_s`를 보면 바로 나온다.

### 아직 답이 없는 것: 동시 환경 수를 늘리면 빨라지는가

위 표는 전부 `NUM_ENVS=4`에서 잰 값이다. 기본값 `NUM_ENVS=16`이 얼마나 빠른지는 아직
재지 않았다. 이 답에 따라 전체 일정이 최대 3배까지 달라진다.

3배라는 숫자는 SkillGen 실행에서 관찰한 값이고 MimicGen에서 확인한 적이 없다. MimicGen은
시도를 만들어 내는 부분이 단일 스레드라 환경을 늘려도 같은 이득이 나오지 않을 수 있다.
일정을 약속할 때는 위 표(환경 4개)를 쓰고, 빨라지면 그때 줄이는 편이 안전하다.

재는 방법은 간단하다. `make smoke RUN_ARGS="-e NUM_ENVS=16"`으로 한 번 돌려 첫 청크
`MANIFEST.json`의 `durations_s.generate`를 환경 4개일 때와 비교하면 된다.
...
`MANIFEST.json`의 `yield` 값도 같이 보면 좋다. 0.14에서 크게 떨어져 있으면 소스 시연 필터나
