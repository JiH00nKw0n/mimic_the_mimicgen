...
`NUM_ENVS=16`, `GEN_PROCS=1`, `RENDER_PROCS=2`, 물리 랜덤화를 켠 상태다.
...
첫째, 물리 랜덤화를 켜도 수율이 떨어지지 않는다. 켜기 전이 15.2%(164시도 중 25성공),
켠 뒤가 14.2%(351시도 중 50성공)로 차이가 통계적으로 유의하지 않다. 실측 물리를 넣는
대가로 데이터를 잃는 일은 없다는 뜻이다.

둘째, **동시 환경 수를 늘려도 생성이 빨라지지 않는다.** `NUM_ENVS=4`에서 시도당 3.17초였고
`NUM_ENVS=16`에서 2.97초다. 6.7% 차이다. 앞서 SkillGen에서 관찰한 3배 이득은 MimicGen에서
재현되지 않았다. MimicGen 오케스트레이터가 시도를 한 번에 하나씩 처리해서, 환경을 늘려도
대부분이 놀고 있는 것으로 보인다. 예상 시간을 계산할 때 이 이득을 기대하면 안 된다.

### 추정한 값

위 편당 31.9초를 그대로 늘린 값이다.
...
- 이 표는 이전 판의 "보수 11.5일 / 낙관 5.1일"을 대체한다. 그 값은 동시 환경 수를 늘리면
  3배 빨라진다는 가정에 기대고 있었고, 위에서 그 가정이 틀린 것으로 확인됐다.
