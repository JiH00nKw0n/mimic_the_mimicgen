...
def main():
    ...
    # hf80k 변경: 예전에는 위반을 보고서에만 적고 종료 코드 0으로 끝냈다. 그러면
    # 계약 형식이 깨진 파일이 그대로 다음 단계로 넘어가고, 성공 표시가 빠진 것 같은
    # 문제를 잡아 줄 유일한 검사가 무력해진다. 위반이 있으면 실패로 끝낸다.
    if issues:
        print(f"[convert] 계약 형식 검사에서 {len(issues)}건이 걸려 실패로 끝낸다",
