...
def build_env_cfg(device: str, table_usd: str, cameras: dict | None = None, num_envs: int = 1):
    ...
    # 생성 환경(src/env/lab_mimic_cfg.py)이 work_surface를 kinematic RigidObjectCfg로
    # 바꿨다. 물리 캘리브레이션이 책상과 큐브 사이 마찰을 이 물체에 써야 하기 때문이다.
    # 그러면 work_surface가 기록된 상태의 rigid_object 목록에 들어오고, 렌더러는 그
    # 목록을 그대로 재생하며 write_root_pose_to_sim을 부른다. 여기가 AssetBaseCfg면
    # 그 함수가 없어 첫 데모에서 AttributeError로 죽는다. 그래서 같은 정의를 쓴다.
    cfg.scene.work_surface = RigidObjectCfg(
