...
4. **(가장 중요) 생성 데이터의 받침 좌표계가 실제 실험실과 180° 반대다.** 이번 렌더
   ...
   원인은 설정값 한 줄의 의미가 버전 변경으로 뒤집힌 것이다. z축 180° 회전을 의도해
   적은 `rot = (0,0,0,1)`이 예전 규약(w,x,y,z)에서는 180° 회전이지만 지금 버전은
   스폰에서 (x,y,z,w)로 읽어 무회전이 된다. 큐브는 절대 좌표로 배치되고 1번 관절이
   ±166°까지 돌아가므로, 로봇이 뒤를 보고 서 있어도 작업은 성공해 오류로 드러나지
   않았다.

   **교정 검증(2026-08-05).** 생성 환경에 `LAB_ROBOT_SPAWN_ROT` 스위치를 넣고 의도했던
   180° 회전으로 생성했더니 1번 관절 사용 범위가 ±2.0~2.4 라디안에서 −0.37~0.44
   라디안으로 줄어 사람 원본과 같은 정면 작업이 됐고, 수율은 25/201 = 12.4%
   (95% CI 8.6~17.7%)로 기존 25/385 = 6.5%(4.4~9.4%)의 1.9배였다(z=2.45, p=0.014).
   남은 일은 이 설정으로 데이터를 다시 생성해 RGB-Action 데이터셋을 다시 만드는 것이다.
